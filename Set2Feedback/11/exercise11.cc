...
int main()                              // HB: lots of CC
{
    ...
    string values;
    for (size_t index = 0; index < (sizeof(input)*8); ++index) // HB: CANON? 
                            // (you can do some wild stuff with loops in this
                            // ex. if you want, but don't use a `<` operator)
    {                       // HB: ^^^^^^^^^^^^^^^^^ this is best put in a 
                            // const variable or even an enum in an ANS
        auto const result = input & 1;          // Store current bit // HB: 
                                // `result` is too generic of a name: what 
                                // does it represent? also, don't use auto
        binary = to_string(result) + binary;    // Append to binary repr. 
                                    // HB: prepend, since it is in front
