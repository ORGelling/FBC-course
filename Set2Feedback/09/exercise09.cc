...
{                                       
    ...
    // HB: give a name that clarifies its use: e.g. `aCount`, `nrAChars` and `amountA`
        
    // HB: for-loop with multiple lines in the body -> use braces (even if it is technically one statement)
    ...
    // HB: only use for-loops when you know the number of iterations in advance
        for (auto const &letter : word) // Looping through letters in word
            // HB: avoid using auto for now (we'll start allowing it in the second half of part II)

            switch (letter)             // Use letters to count iterators // HB: I'm not sure what you mean by this?
            {
                case 'a':               // merged entrypoint for LC and UC
                case 'A':               // so no falling through // HB: quite the opposite: you ARE using a fallthrough, just an intentional one
                    ++achars;           // Count a's
                break;                  
